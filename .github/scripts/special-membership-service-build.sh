...
# build
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'build' -- \
    mvn clean verify -pl special-membership-service -Pcode-coverage -Pstatic-code-analysis
# verify pacts
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'verify pacts' -- \
...
# create pacts
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'create pacts' -- \
    mvn verify -pl special-membership-service -Pconsumer-pacts
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'publish pacts' -- \
...
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'tag pacts for production' -- \
...
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'verify published pacts' -- \
...
# can-i-deploy
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'pact verify can-i-deploy' -- \
...
buildevents cmd $WORKFLOW_TRACE_ID $STEP_ID 'tag pacts as production' -- \
